...
housing_ma_tx <- filter(housing, State == "MA" | State == "TX")

ggplot(housing_ma_tx, mapping = aes(y = Home.Value, x = Date)) + geom_point()


housing.2001 <- subset(housing, Date == 2001.25) 

# Create a scatter plot that's x-axis is Land.Value and y-axis is Structure.Cost
ggplot(housing, mapping = aes(y = Structure.Cost, x = Land.Value)) + geom_point()
